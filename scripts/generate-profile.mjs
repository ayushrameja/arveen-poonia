...
// Convert the enhanced YouTube profile photo into responsive colored ASCII.
...
const source = new URL('../assets/arveen-profile-enhanced.png', import.meta.url).pathname;
...
await sharp(portrait).webp({ quality: 90, alphaQuality: 95 }).toFile(new URL('arveen-profile-ascii.webp', directory).pathname);
await sharp(portrait).resize(1200).webp({ quality: 90, alphaQuality: 95 }).toFile(new URL('arveen-profile-ascii-mobile.webp', directory).pathname);
console.log(`Profile portrait: ${cells.length / 2} colored ASCII glyphs.`);
