...
// Offline conversion: the browser receives two images, never a glyph loop.
...
const { data } = await sharp(new URL('../assets/krishna-color-source.png', import.meta.url).pathname)
...
await sharp(portrait).webp({ quality: 88, alphaQuality: 95 }).toFile(new URL('krishna-color-ascii.webp', directory).pathname);
await sharp(portrait).resize(800).webp({ quality: 88, alphaQuality: 95 }).toFile(new URL('krishna-color-ascii-mobile.webp', directory).pathname);
// Decode/composite only the small garment strip, not another full-size portrait.
await sharp(svg(changes)).extract({ left: 0, top: 1032, width: 1440, height: 300 })
  .webp({ lossless: true }).toFile(new URL('krishna-texture.webp', directory).pathname);
console.log(`Generated ${cells.length} colored ASCII glyphs with ${changes.length} sparse clothing variations.`);
